...
public class ForumPostServiceImpl implements IForumPostService 
{
    ...
    {
        ForumPost forumPost = forumPostMapper.selectForumPostByPostId(postId);
        enrichDisplayFields(forumPost);
        return forumPost;
    }
    ...
    {
        List<ForumPost> list = forumPostMapper.selectForumPostList(forumPost);
    ...
    private void enrichDisplayFields(ForumPost forumPost)
    {
        if (forumPost == null)
        {
            return;
        }
        if ((forumPost.getUserId() == null || forumPost.getUserId() == 0L) && StringUtils.isBlank(forumPost.getUserName()))
        {
            forumPost.setUserName(forumPost.getCreateBy());
        }
    }
}
